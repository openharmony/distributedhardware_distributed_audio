...
#include "audio_transport_start_status_test.h"
...
void AudioTransportStartStatusTest::SetUpTestCase(void)
{
}

void AudioTransportStartStatusTest::TearDownTestCase(void)
{
}

void AudioTransportStartStatusTest::SetUp(void)
{
    ...
    audioStatus_ = std::make_shared<AudioTransportStartStatus>(stateContext_);
}

void AudioTransportStartStatusTest::TearDown(void)
{
...
/**
 * @tc.name: transport_start_test_001
 * @tc.desc: Verify start action when status is start.
 * @tc.type: FUNC
 * @tc.require: AR000H0E5U
 */
HWTEST_F(AudioTransportStartStatusTest, transport_start_test_001, TestSize.Level1)
{
    EXPECT_EQ(DH_SUCCESS, audioStatus_->Start(audioChannel_, processor_));
}
