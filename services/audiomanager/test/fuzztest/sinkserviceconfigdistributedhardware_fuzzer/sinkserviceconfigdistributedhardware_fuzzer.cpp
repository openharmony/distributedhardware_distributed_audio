...
void FuzzConfigDistributedHardwareRandom(
    const std::shared_ptr<DAudioSinkService> &svc, FuzzedDataProvider &fdp)
{
...
void FuzzConfigDistributedHardwareWithEnableInit(
    const std::shared_ptr<DAudioSinkService> &svc, FuzzedDataProvider &fdp)
{
    std::string devId = fdp.ConsumeRandomLengthString();
    std::string dhId = fdp.ConsumeRandomLengthString();
...
void SinkServiceConfigDistributedHardwareFuzzTest(const uint8_t *data, size_t size)
{
    ...
    if (route == 0) {
        FuzzConfigDistributedHardwareRandom(svc, fdp);
    } else {
        FuzzConfigDistributedHardwareWithEnableInit(svc, fdp);
    }
}
}
}
...
{
    OHOS::DistributedHardware::SinkServiceConfigDistributedHardwareFuzzTest(data, size);
    return 0;
}
