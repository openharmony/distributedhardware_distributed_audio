...
int32_t AudioManagerInterfaceImpl::AddAudioDevice(const std::string &adpName, const uint32_t devNo,
    const std::string &caps, const sptr<IDAudioCallback> &callback)
{
    DHLOGI("Add audio device name: %s, device: %d.", GetAnonyString(adpName).c_str(), devNo);
    std::lock_guard<std::mutex> adpLck(adapterMapMtx_);
    auto adp = mapAudioAdapter_.find(adpName);
    if (adp == mapAudioAdapter_.end()) {
        int32_t ret = CreateAdapter(adpName, devNo, callback);
        if (ret != DH_SUCCESS) {
    ...
    int32_t ret = adp->second->AddAudioDevice(devNo, caps);
    if (ret != DH_SUCCESS) {
    ...
    DAudioDevEvent event = { adpName,
                             ...
                             0,
                             adp->second->GetVolumeGroup(devNo),
                             adp->second->GetInterruptGroup(devNo) };
    ret = NotifyFwk(event);
    if (ret != DH_SUCCESS) {
...
int32_t AudioManagerInterfaceImpl::RemoveAudioDevice(const std::string &adpName, const uint32_t devNo)
{
    DHLOGI("Remove audio device name: %s, device: %d.", GetAnonyString(adpName).c_str(), devNo);
    std::lock_guard<std::mutex> adpLck(adapterMapMtx_);
    auto adp = mapAudioAdapter_.find(adpName);
    if (adp == mapAudioAdapter_.end() || adp->second == nullptr) {
        DHLOGE("Audio device has not been created  or is null ptr.");
    ...
    int32_t ret = adp->second->RemoveAudioDevice(devNo);
    if (ret != DH_SUCCESS) {
        DHLOGE("Remove audio device failed, adapter return: %d.", ret);
        return ret;
    }

    DAudioDevEvent event = { adpName, devNo, HDF_AUDIO_DEVICE_REMOVE, 0, 0, 0 };
    ret = NotifyFwk(event);
    if (ret != DH_SUCCESS) {
        DHLOGE("Notify audio fwk failed, ret = %d.", ret);
    }
...
int32_t AudioManagerInterfaceImpl::NotifyFwk(const DAudioDevEvent &event)
{
    DHLOGD("Notify audio fwk event(type:%d, adapter:%s, pin:%d).", event.eventType,
        GetAnonyString(event.adapterName).c_str(), event.devNo);
    std::stringstream ss;
    ss << "EVENT_TYPE=" << event.eventType << ";NID=" << event.adapterName << ";PIN=" << event.devNo << ";VID=" <<
